...
class Evaluator_nsm:
    ...
    def send_sparql_query(self,ids):
    ...
    def get_instances(self,ids,isAnswers = False):
        try:
            result = self.send_sparql_query(ids)
            # Process the result here
            instances = []
            for r in result["results"]["bindings"]:
                instances.append(r["sLabel"]["value"])
            return instances if (isAnswers and (len(ids) == len(instances))) or (not isAnswers) else None
    ...
    def evaluate(self, valid_data, test_batch_size=20, write_info=False):
        ...
        for iteration in tqdm(range(num_epoch)):
            ...
            for batch_id in range(batch_size):
                ...
                if write_info:
                    ...
                    if not wroteQandA:
                        qstion = []
                        for word in query_text[batch_id]:
                            if word == 6718:
                                break
                            qstion.append(self.id2word[word])

                        answer_instances = [self.id2entity[int(a)] for a in answers]

                        candidate_instances = [self.id2entity[int(e[0])] for e in candidate2prob]

                        ans = self.get_instances(answer_instances,True)
                        cand = self.get_instances(candidate_instances)
                        listOfQandA.append(f"{' '.join(qstion)+'?'} \n Answers :: {str(ans)} \n Candidates :: {str(cand)} \n Candidate ids :: {str(candidate_instances)}\n")
            ...
            # already dumped answers
            wroteQandA = True
            with open("q&ADumpNSMBase.txt","w") as file:
                for q in listOfQandA:
                    file.write(q)
        ...
        if write_info:
            ...
            self.file_write = None
        return np.mean(f1s), np.mean(hits), np.mean(recalls)

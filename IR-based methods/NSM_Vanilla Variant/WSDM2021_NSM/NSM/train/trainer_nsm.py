...
class Trainer_KBQA(object):
        ...
        for epoch in range(start_epoch, end_epoch + 1):
            ...
            if (epoch + 1) % eval_every == 0 and epoch + 1 > 0:
                if self.model_name == "back":
                    ...
                    eval_f1, eval_h1, eval_recall = self.evaluate(self.valid_data, self.test_batch_size, mode="teacher")
                self.logger.info("EVAL F1: {:.4f}, H1: {:.4f}, Recall: {:.4f}".format(eval_f1, eval_h1, eval_recall))
    ...
    def evaluate_best(self, mode):
        ...
        eval_f1, eval_h1, eval_recall = self.evaluate(self.test_data, self.test_batch_size, mode="teacher", write_info=False)
    ...
    def evaluate_single(self, filename):
        if filename is not None:
            self.load_ckpt(filename)
        test_f1, test_hits, test_recall = self.evaluate(self.test_data, self.test_batch_size, mode=self.mode, write_info=True)
        self.logger.info("TEST F1: {:.4f}, H1: {:.4f}, Recall: {:.4f}".format(test_f1, test_hits, test_recall))

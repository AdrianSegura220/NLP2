...
from transformers import AutoTokenizer, BertModel, BartModel, RobertaModel, ElectraModel, DistilBertModel
...
class Parser(nn.Module):
    def __init__(self, vocab, dim_word, dim_hidden, max_dec_len=20, max_inp=3):
        ...
        self.word_embeddings = nn.Embedding(num_words, self.dim_word)
        self.word_dropout = nn.Dropout(0.2)
        self.question_encoder = GRU(self.dim_word, dim_hidden, num_layers=2, dropout=0.2)

        self.func_embeddings = nn.Embedding(num_func, self.dim_word)
        self.decoder = GRU(self.dim_word, dim_hidden, num_layers=2, dropout=0.2)
        ...
        self.inp_embeddings = nn.Embedding(num_words, self.dim_word)
        self.inp_decoder = GRU(self.dim_word + dim_hidden, dim_hidden, num_layers=2, dropout=0.2)
    ...
    def forward(self, questions, programs=None, inputs=None):
        ...
        """
        #print(list(self.vocab['word_idx_to_token'].items())[:50])
        list_sentences = []
        for i,question in enumerate(questions):
            ...
            for j,word in enumerate(question):
                if word.item() == 0:
                    break
                list_sentences[i] += ''+self.vocab['word_idx_to_token'][word.item()] if j == 0  else ' '+self.vocab['word_idx_to_token'][word.item()]

        #print(query_text.shape)
        
        own_inputs = self.tokenizer(list_sentences, return_tensors="pt",max_length=len(questions[0])+2, truncation=True, padding='max_length')
        input_ids = own_inputs['input_ids']
        attention_mask = own_inputs['attention_mask']

        if self.modelType == "bart" or self.modelType == "bert":
            outputs = self.model(input_ids.to('cuda:0'), attention_mask.to('cuda:0'))
        else:
            token_type_ids = own_inputs['token_type_ids']
            outputs = self.model(input_ids.to('cuda:0'), attention_mask.to('cuda:0'), token_type_ids.to('cuda:0'))

        own_hidden_embeddings = outputs.last_hidden_state[:,1:-1,:]
        ...
        q_word_h, q_embeddings, q_hn = self.question_encoder(own_hidden_embeddings, question_lens)
